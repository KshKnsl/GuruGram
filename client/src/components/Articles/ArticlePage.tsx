...
const ArticlePage: React.FC<ArticlePageProps> = ({ title, content, author, date, comments, onAddComment }) => {
  return (
    ...
    >
      ...
      >
        <UserCircleIcon className="w-5 h-5 mr-2 dark:text-gray-400" />
        <span className="mr-4"><MentorName id={author} /></span>
        <CalendarIcon className="w-5 h-5 mr-2 dark:text-gray-400" />
        <span>{date}</span>
      </motion.div>
      <motion.div
        initial={{ opacity: 0 }}
        ...
        className="prose max-w-none mb-8 dark:prose-dark dark:text-white"
