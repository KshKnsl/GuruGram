...
const ChatHeader = () => {
  ...
  const handleRatingSubmit = async () => {
    const endpoint = `${import.meta.env.VITE_BACKEND_URL}/api/mentor/updateRating`;
    try {
      if (selectedUser) {
        await axios.post(endpoint, { _id: selectedUser._id, rating });
      }
  ...
  return (
    <div className="p-4 border-b border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm rounded-lg">
      ...
      <div className="flex items-center gap-4">
        ...
        <ReactStars
          count={5}
          onChange={(newRating: SetStateAction<number>) => setRating(newRating)}
          size={24}
          activeColor="#ffd700"
        />
        <button
